...
from manager.OSNetv2 import osnet_x1_0, osnet_att_x1_0
from manager.RMNet import RMNet
from manager.ResNet import ResNet, BasicBlock
from manager.utility import ConvFC, CenterLoss, AMSoftmax, CrossEntropyLossLS, TripletLoss, AttentionConvBlock
from manager.base_manager import BaseManager
import logging
logger = logging.getLogger("logger")

class AttentionManager(BaseManager):
    ...
    def _make_loss(self):
        ...
        center_loss = CenterLoss(feat_dim, self.cfg.DB.NUM_CLASSES, len(self.cfg.MODEL.GPU) > 0 and torch.cuda.is_available())        
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.NAME == 'osnet':
            ...
            if cfg.MODEL.PRETRAIN == "outside":
                ...
                self.backbone = osnet_att_x1_0(num_classes=cfg.DB.NUM_CLASSES, pooling=cfg.MODEL.POOLING, task=cfg.MODEL.TASK)        
        else:
            logger.info("{} is not supported".format(cfg.MODEL.NAME))
            sys.exit(1)
