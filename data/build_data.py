...
class build_DFKP_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            hm = np.zeros((num_classes, output_res_h, output_res_w), dtype=np.float32)
            hm_hp          = np.zeros((num_joints , output_res_h, output_res_w), dtype=np.float32)
                ...
                for j in range(num_joints):
                    if pts[j, 2] > 0:
                        ...
                        if self.split == 'train':
                            ...
                            if pts[j, 0] >= 0 and pts[j, 0] < output_res_w and pts[j, 1] >= 0 and pts[j, 1] < output_res_h:
                                kps[k, j * 2: j * 2 + 2] = pts[j, :2] - ct_int
                                kps_mask[k, j * 2: j * 2 + 2] = 1
                                pt_int = pts[j, :2].astype(np.int32)
                                hp_offset[k * num_joints + j] = pts[j, :2] - pt_int
                                hp_ind[k * num_joints + j] = pt_int[1] * output_res_w + pt_int[0]
                                hp_mask[k * num_joints + j] = 1
                                draw_gaussian(hm_hp[j], pt_int, hp_radius)
                draw_gaussian(hm[cls_id], ct_int, radius)
