...
class ConvLayer(nn.Module):
    ...
    def __init__(self, name, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1):
...
class OSBlock(nn.Module):
    ...
    def __init__(self, name, in_channels, out_channels, bottleneck_reduction=4, **kwargs):
        super(OSBlock, self).__init__()
        self.g_name = name
        mid_channels = out_channels // bottleneck_reduction
        self.conv1 = Conv1x1(name + '/conv1', in_channels, mid_channels)
        self.conv2a = LightConv3x3(name + '/conv2a/1', mid_channels, mid_channels)
        self.conv2b = nn.Sequential(
            LightConv3x3(name + '/conv2b/1', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2b/2', mid_channels, mid_channels),
        )
        self.conv2c = nn.Sequential(
            LightConv3x3(name + '/conv2c/1', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2c/2', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2c/3', mid_channels, mid_channels),
        )
        self.conv2d = nn.Sequential(
            LightConv3x3(name + '/conv2d/1', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2d/2', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2d/3', mid_channels, mid_channels),
            LightConv3x3(name + '/conv2d/4', mid_channels, mid_channels),
        )
        self.gate = ChannelGate(name + '/2a/gate', mid_channels)
        self.conv3 = Conv1x1Linear(name + '/conv3', mid_channels, out_channels)
        self.downsample = None
        if in_channels != out_channels:
            self.downsample = Conv1x1Linear(name + '/downsample', in_channels, out_channels)
    ...
    def generate_caffe_prototxt(self, caffe_net, layer):
        ...
        layer = generate_caffe_prototxt(self.lrelu, caffe_net, layer)
        return layer
...
class OSNet(nn.Module):
    ...
    def __init__(self, name, num_classes, blocks, layers, channels, feature_dim=512, task='object', num_keypoints=0, **kwargs):
        ...
        self.conv = nn.Sequential(
            ConvLayer(name + '/block0', 3, channels[0], 7, stride=2, padding=3),
        ...
        self.fpn3 = Conv1x1(self.g_name + '/fpn3', channels[3], 256)
        ...
        self.size_reg   = Regressor(self.g_name + '/object_size', 128, 2)

        if task == 'keypoint':
            assert num_keypoints != 0
            self.kp_hm_reg     = Regressor(self.g_name + '/keypoint_heatmap', 128, num_keypoints)
            self.kp_offset_reg = Regressor(self.g_name + '/keypoint_offset', 128, 2)
            self.kp_loc_reg    = Regressor(self.g_name + '/keypoint_location', 128, num_keypoints * 2)

        self._init_params()

    def _make_layer(self, name, block, layer, in_channels, out_channels, reduce_spatial_size):
        layers = []
        
        layers.append(block(name + '/0', in_channels, out_channels))
        for i in range(1, layer):
            layers.append(block(name + '/{}'.format(i), out_channels, out_channels))
        
        if reduce_spatial_size:
            layers.append(
                nn.Sequential(
                    Conv1x1(name + '/reduce', out_channels, out_channels),
...
# Instantiation
##########
def osnet_x1_0(name, num_classes=1000, **kwargs):
    # standard size (width x1.0)
    return OSNet(name, num_classes, blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
