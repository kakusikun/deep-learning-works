...
class build_cocokp_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        kp_reg      = np.zeros((self.max_objs * num_joints, 2), dtype=np.float32)
        kp_ind         = np.zeros((self.max_objs * num_joints), dtype=np.int64)
        kp_mask        = np.zeros((self.max_objs * num_joints), dtype=np.int64)
