...
def build_coco_loader(cfg):
    ...
    loader = {}
    if cfg.DB.USE_TRAIN:
        train_dataset = get_dataset(cfg.DB.DATASET)(data.handle['train'], data.index_map['train'], split='train', output_stride=cfg.MODEL.STRIDE)
    ...
    if cfg.DB.USE_TEST:
        val_dataset = get_dataset(cfg.DB.DATASET)(data.handle['val'], data.index_map['val'], split='val', output_stride=cfg.MODEL.STRIDE)
